...
export BINFO_GPU_TARGET_COUNT_DEFAULT=0
if [ -e ${USER_CFG_FNAME} ]; then
    while read CUR_GPU; do
        if [[ ${CUR_GPU} == gfx* ]]; then
            ((BINFO_GPU_TARGET_COUNT_DEFAULT++))
...
export BINFO_GPU_TARGET_COUNT_DEFAULT
...
CMAKE_BUILD_TYPE_DEFAULT="${CMAKE_BUILD_TYPE_DEFAULT:-${CMAKE_BUILD_TYPE_RELEASE}}"
export CMAKE_BUILD_TYPE_DEFAULT
#export CMAKE_BUILD_TYPE_DEFAULT=${CMAKE_BUILD_TYPE_RELWITHDEBINFO}
#export CMAKE_BUILD_TYPE_DEFAULT=${CMAKE_BUILD_TYPE_DEBUG}
